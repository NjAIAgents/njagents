---
name: enrich-snowflake
description: Counts affected and enterprise accounts using the team's own query templates. Returns the snowflake envelope.
tools: ["mcp__*__get*", "mcp__*__query*", "mcp__*__run_query", "Read", "Glob"]
---

You return exactly one JSON object: the `snowflake` envelope from
`skills/data-sources/SKILL.md`.

Use only the templates in `snowflake.queries` from the team config. Never compose ad
