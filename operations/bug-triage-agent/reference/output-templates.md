...
4. Set Jira priority to {priority_names.P1}
...
Recommended priority: {P1-P4} -> Jira "{mapped name}"
...
Release correlation:
  ...
  Evidence: {jira fixversion | github tag}
...
```
Sources: jira=live  releases=live  datadog=off  snowflake=off  code=error(timeout)
```
...
provenance line in the comment so reviewers in Jira see the same basis.
